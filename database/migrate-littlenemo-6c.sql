...
create table log_issues engine=MyISAM select * from GCDOnline.LogIssues;
-- drop columns won't need for porting
alter table log_issues drop column ip,
...
-- don't port issues from series that have been deleted
...
insert into log_issues2
    select min(id) as id, volumenum, seriesid, pub_date, price, modified, modtime,
           key_date, issue, issueid, userid
    from log_issues
    group by volumenum, seriesid, pub_date, price, key_date, issue, issueid;

-- 193953 records reduced to 155241 (20% less)

-- use 2009-11-01 00:00:00 as placeholder (max date in table is 2009-10-01,
-- first new site date is 2009-12-07)
